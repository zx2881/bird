<template>
  <div class="overview-page">
    <div v-if="store.overviewLoading || !store.loaded" class="overview-loading">正在加载概览数据…</div>
    ...
    <section class="map-section reveal-card">
      <div class="map-header">
        ...
        <span class="chart-card-badge map-badge">{{ overviewBirds.filter(b => b.lat != null && b.lng != null).length }} 有坐标</span>
      </div>
...
const threatenedCount = computed(() => {
  ...
  overviewBirds.value.forEach(bird => {
    if (bird.status) counts[bird.status] = (counts[bird.status] || 0) + 1
  })
...
onMounted(async () => {
  await Promise.all([
    store.loadOverviewData(),
    store.loaded ? Promise.resolve() : store.loadData()
  ])
  await nextTick()
  setTimeout(() => {
